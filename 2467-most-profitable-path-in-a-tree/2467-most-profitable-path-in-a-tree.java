class Solution
{
    ...
    public boolean findBobPath(int node, int time, List<Integer>[] tree, int[] bobTime, int bob)
    {
        bobTime[node] = time;
        if(node == 0)
        {
    ...
    public int dfs(int node, int parent, List<Integer>[] tree, int[] amount, int time, int[] bobTime)
    {
